...
const DEFAULTS = {
  ...
  // AI voice expense logging — cost controls. Two independent
  // protections, deliberately not just one: a per-customer monthly
  // allowance bounds what any single customer can cost you even if
  // usage patterns are wildly uneven, and a global monthly ceiling
  // bounds total exposure even if the per-customer math is ever wrong
  // (a bug, an unexpectedly high adoption rate) — see lib/aiUsage.js for
  // where both are actually enforced. Real OpenAI account-level spend
  // limits should also be configured directly in OpenAI's own dashboard
  // as a further backstop — that's outside this app entirely, so there's
  // deliberately no setting for it here.
  voice_free_monthly_limit: "5", // free voice logs per customer per calendar month
  voice_monthly_spend_ceiling_usd: "20", // once this month's total estimated cost (all customers combined) reaches this, the feature disables itself until the next calendar month — see lib/aiUsage.js
  // Empty means no active promotion. Set to a future date (YYYY-MM-DD) to
  // make voice logging unlimited and free for every customer until that
  // date, ignoring voice_free_monthly_limit entirely — intended for a
  // time-limited launch promotion. voice_monthly_spend_ceiling_usd still
  // applies even during a promo — a promotion is meant to waive the
  // per-customer allowance, not the safety net protecting against a
  // runaway bill.
  voice_promo_free_until: "",

  // Lets a customer buy more voice logging right now via a payment
  // gateway (MitaPesa as merchant, their own bank card) rather than wait
  // for their free monthly allowance to reset — see
  // services/paymentGateway and lib/aiUsage.js. Purchased credits never
  // expire, unlike the monthly free allowance above. Price is in TZS
  // (this app's one real anchor currency), admin-configurable so pricing
  // can be tuned without a code change.
  voice_credit_pack_size: "20", // how many extra voice logs one purchase unlocks
  voice_credit_pack_price_tzs: "5000",
  // Same simulated-failure-injection pattern as cbs_simulated_failure_rate
  // and tips_simulated_failure_rate — "0"-"100" — lets the decline-
