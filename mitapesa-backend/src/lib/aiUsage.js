...
// A customer's full voice-credit picture for admin visibility — this is
// exactly what was missing when a customer's leftover credits from
// earlier testing silently kept covering requests after the free
// allowance was intentionally lowered, with no way for an admin to see
// why. remaining is derived the same way findAvailableCredit sums it,
// not duplicated separately, so this can never drift out of sync with
// what's actually enforced.
async function getVoiceCreditSummary(userId) {
  const purchases = await prisma.voiceCreditPurchase.findMany({
    where: { userId },
...
// Marks every one of a customer's unused, succeeded credits as fully
// consumed — for clearing out leftover test-purchase credits so the free
// monthly allowance can actually be tested cleanly, without a purchase
// made days or weeks earlier still silently covering every request.
// Deliberately doesn't delete the purchase records themselves: what was
// actually charged (even in testing, even to the mock gateway) stays as
// a real, honest history — this only changes what's left to spend, not
// what happened.
async function clearVoiceCredits(userId) {
  ...
  // hot path.
  const purchases = await prisma.voiceCreditPurchase.findMany({ where: { userId, status: "succeeded" } });
...
module.exports = { currentMonthKey, checkVoiceUsageAllowed, recordVoiceUsage, purchaseVoiceCredits, getVoiceCreditSummary, clearVoiceCredits };
