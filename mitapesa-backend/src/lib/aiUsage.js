...
async function checkVoiceUsageAllowed(userId) {
  ...
  if (credit) {
    return { allowed: true, usingCreditId: credit.id };
  }
  ...
  return {
    allowed: false,
    reason: `You've used your ${freeLimit ?? 0} free voice logs for this month. Buy ${packSize ?? 20} more for TZS ${(packPrice ?? 5000).toLocaleString()}, or add this expense manually and check back next month.`,
...
async function purchaseVoiceCredits(userId) {
  ...
  return { success: true, creditsPurchased: packSize, purchaseId: purchase.id };
}
