...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (req.query.format === "csv") {
      return sendCsv(res, `mitapesa-statement-${req.params.userId}-${from.toISOString().slice(0, 10)}-to-${to.toISOString().slice(0, 10)}.csv`, statementToCsv(statement));
    }
    res.json(statement);
  })
);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await notifyUser(user.id, { type: "account_blocked", title: "Account access restricted", message: "Your MitaPesa account has been temporarily blocked. Please contact support.", url: "/support" });
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await notifyUser(user.id, { type: "account_unblocked", title: "Account access restored", message: "Your MitaPesa account is active again.", url: "/" });
    res.json({ user: publicUser(updated) });
  })
);
...
// GET /admin/ai-usage?feature=analytics — this calendar month's actual
// spend and usage for the given AI feature (defaults to "voice" for
// backward compatibility with existing callers), against that feature's
// own configured ceiling — so an admin can see how close the account is
// to the automatic cutoff (see lib/aiUsage.js) before it actually
// happens, rather than only finding out once customers start getting
// turned away. Voice and analytics are tracked as two fully independent
// budgets sharing the same underlying OpenAI account — this endpoint
// reports one feature at a time, never a combined total, since the
// settings themselves (ceiling, free limit, pricing) are independent
// per feature too. Any admin tier can view, same as /settings above —
// this is reporting, not a change that needs admin_super.
router.get(
  "/ai-usage",
  asyncHandler(async (req, res) => {
    ...
    const [totals, topUsersRaw, ceilingUsd, freeLimit, promoUntil, revenueThisMonth] = await Promise.all([
      ...
      getSetting(`${feature}_free_monthly_limit`),
      getSetting(`${feature}_promo_free_until`),
      // This month's actual revenue from customers buying more usage of
      // this feature — the counterpart to totalCostUsd above, so an
      // admin can see what was spent vs. what was earned at a glance, in
      // the same place. Same status: "succeeded" filter the unified fee
      // report's own revenue queries use, for consistency between the two.
    ...
    res.json({
      ...
      freeMonthlyLimit: Number(freeLimit) || 0,
      promoFreeUntil: promoUntil || null,
      revenueThisMonthTzs: Number(revenueThisMonth._sum.amountPaidTzs || 0),
      revenuePurchasesThisMonth: revenueThisMonth._count,
      topUsers: topUsersRaw.map((u) => ({
        userId: u.userId,
        name: userById[u.userId] ? `${userById[u.userId].firstName} ${userById[u.userId].lastName}` : "Unknown",
        email: userById[u.userId]?.email || null,
...
// GET /admin/onboarding-tips — every tip, active and inactive, for the
// admin management screen. Any admin tier can view.
router.get(
  "/onboarding-tips",
  asyncHandler(async (req, res) => {
    const tips = await prisma.onboardingTip.findMany({ orderBy: [{ tourStep: "asc" }, { key: "asc" }] });
    res.json({ tips });
  })
);
...
const tipBodySchema = z.object({
  ...
  textSw: z.string().min(1).max(500).optional(),
  tourStep: z.number().int().positive().nullable().optional(),
  active: z.boolean().optional(),
});

// POST /admin/onboarding-tips — create a new tip. key, textEn and
// textSw are all required for a new tip (Zod's .optional() above exists
// so the same schema can be reused for PATCH's partial updates below).
// admin_support and above — this is customer-facing copy, not a
// financial or security setting, so it doesn't need admin_super.
router.post(
  "/onboarding-tips",
  requireAdminRole("admin_support"),
  asyncHandler(async (req, res) => {
    const body = z.object({ key: z.string().min(1).max(100), textEn: z.string().min(1).max(500), textSw: z.string().min(1).max(500), tourStep: z.number().int().positive().nullable().optional() }).parse(req.body);
    const tip = await prisma.onboardingTip.create({ data: { ...body, updatedByUserId: req.userId } });
    await writeAudit(req.userId, "admin.onboarding_tip_created", { ip: req.ip, key: tip.key });
    res.json(tip);
  })
);

// PATCH /admin/onboarding-tips/:key — edit an existing tip's wording,
// tour position, or active status. This is deliberately the only way
// tip content changes: never hardcoded in the app itself, so correcting
// a phrase that doesn't sound natural in the Swahili customers actually
// speak takes effect immediately, for every customer, with no app
// update or code deploy.
router.patch(
  "/onboarding-tips/:key",
  requireAdminRole("admin_support"),
  asyncHandler(async (req, res) => {
    const { key } = req.params;
    const updates = tipBodySchema.omit({ key: true }).parse(req.body);
    const existing = await prisma.onboardingTip.findUnique({ where: { key } });
    if (!existing) throw notFound("Tip not found");
    const tip = await prisma.onboardingTip.update({ where: { key }, data: { ...updates, updatedByUserId: req.userId } });
    await writeAudit(req.userId, "admin.onboarding_tip_updated", { ip: req.ip, key, updates });
    res.json(tip);
  })
);

// DELETE /admin/onboarding-tips/:key — removes a tip entirely. Prefer
// PATCH active:false for a tip that might come back (retires it without
// losing the wording); this is for a tip that was created in error or
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    const existing = await prisma.onboardingTip.findUnique({ where: { key } });
    if (!existing) throw notFound("Tip not found");
    await prisma.onboardingTip.delete({ where: { key } });
    await writeAudit(req.userId, "admin.onboarding_tip_deleted", { ip: req.ip, key });
    res.json({ ok: true });
  })
);
