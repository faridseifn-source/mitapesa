...
// GET /admin/ai-usage — this calendar month's actual voice-logging spend
// and usage, against the configured ceiling from voice_monthly_spend_
// ceiling_usd — so an admin can see how close the account is to the
// automatic cutoff (see lib/aiUsage.js) before it actually happens,
// rather than only finding out once customers start getting turned away.
// Any admin tier can view, same as /settings above — this is reporting,
// not a change that needs admin_super.
router.get(
  "/ai-usage",
  asyncHandler(async (req, res) => {
    ...
    const [totals, topUsersRaw, ceilingUsd, freeLimit, promoUntil, revenueThisMonth] = await Promise.all([
      ...
      getSetting("voice_monthly_spend_ceiling_usd"),
      getSetting("voice_free_monthly_limit"),
      getSetting("voice_promo_free_until"),
      // This month's actual revenue from customers buying more voice
      // logging — the counterpart to totalCostUsd above, so an admin can
      // see what was spent vs. what was earned from this feature at a
      // glance, in the same place. Same status: "succeeded" filter the
      // unified fee report's voiceCreditSales query uses, for consistency
      // between the two.
