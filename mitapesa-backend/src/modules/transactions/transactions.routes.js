...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (req.query.format === "csv") {
      ...
      let out = csvRow(["MitaPesa Transaction Export"]);
      ...
      return sendCsv(res, `mitapesa-transactions-${from}-to-${to}.csv`, out);
    }
...
// Bulk creation — used both for the original use case (bank statement
// ingest, already parsed client-side) and for saving several voice-logged
// expenses from one transcript at once. "manual" is txInput's own default
// when a caller doesn't set source at all, which is indistinguishable from
// "didn't care" here — mapped to "import" to preserve this route's
// original behavior for that case. Anything explicitly set (e.g. "voice")
// is respected as-is, already validated by txInput's schema.
router.post(
  "/bulk",
  asyncHandler(async (req, res) => {
    ...
    const created = await prisma.$transaction(
      items.map((input) =>
        prisma.transaction.create({
          data: { ...input, date: new Date(input.date), loggedByUserId: req.userId, loggedByName: requester.firstName, source: input.source === "manual" ? "import" : input.source },
        })
      )
    );
...
// between MitaPesa's ledger and the verified movement that channel
