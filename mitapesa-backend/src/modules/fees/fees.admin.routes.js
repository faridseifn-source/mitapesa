...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Revenue from AI features specifically (currently just voice expense
    // logging — a customer buying more usage than their free monthly
    // allowance, via the payment gateway; see lib/aiUsage.js). Kept as
    // its own distinct figure rather than folded into totalMitaPesaFee
    // above, since it's a genuinely different kind of revenue — a
    // feature purchase, not a transaction fee — the same reasoning
    // bundleSalesRevenue already applies just above this. status:
    // "succeeded" only, matching how totalMitaPesaFee above only counts
    // actually-collected fees, not attempted-but-failed ones.
    const voiceCreditSales = await prisma.voiceCreditPurchase.aggregate({
