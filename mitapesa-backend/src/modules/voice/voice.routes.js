...
const parseRequestSchema = z.object({
  ...
  durationSeconds: z.number().positive().optional(), // the client's own recording length, used as a cost-estimation fallback — see OpenAiTranscriptionProvider
...
// GET /voice/usage-status — the same allowance/ceiling/credit check
// /parse itself enforces, but on its own, with no audio involved. The
// client calls this the moment the voice logging sheet opens, so a
// customer who's already exhausted their free allowance sees that (and
// the option to buy more) immediately — before recording anything, not
// after wasting a recording only to be told no at the very end. Cheap
// and read-only (a handful of DB queries, no OpenAI call, no charge), so
// no rate limiting needed the way /parse and /credits/purchase have.
router.get(
  "/usage-status",
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Checked before any real provider call is made — the whole point of
    ...
    if (input.audioBase64) {
      try {
        const { text, estimatedCostUsd } = await getTranscriptionProvider().transcribe({
        ...
        transcript = text;
        totalCostUsd += estimatedCostUsd || 0;
      } catch (err) {
      ...
      if (!transcript) {
        ...
        // never fabricating a receipt read from a blank image. The
        // transcription call still genuinely happened and cost real
        // money even though nothing came of it, so it still counts
    ...
    try {
      ...
      // The transcription call (if this request went through audio) did
...
// POST /voice/credits/purchase — charges the customer via the configured
// payment gateway (see services/paymentGateway) for a pack of extra voice
// logs, so they don't have to wait for their free monthly allowance to
// reset. No request body: pack size and price are admin-configured
// settings, not something the client can override — a customer always
// pays exactly what's currently configured, never a client-supplied
// amount. Rate limited the same as /parse, for the same reason: a real
// gateway call (even the mock one, once a real one exists) shouldn't be
// retriable in a tight loop.
router.post(
  "/credits/purchase",
  parseLimiter,
  asyncHandler(async (req, res) => {
