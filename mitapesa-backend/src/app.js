...
// Render sits the app behind one reverse proxy/load balancer, which sets
// X-Forwarded-For — without telling Express to trust that one hop,
// express-rate-limit can't reliably tell clients apart (confirmed
// happening in production logs: ERR_ERL_UNEXPECTED_X_FORWARDED_FOR on
// every rate-limited request), and req.ip elsewhere (audit log entries,
// for instance) would reflect Render's internal proxy address rather than
// the real client. `1` means trust exactly one hop — correct for a single
// proxy layer like Render's, and deliberately not a wildcard/higher
// number, which would trust IPs an attacker could spoof further upstream.
