...
const BASE_URL = import.meta?.env?.VITE_API_URL || "https://mitapesa-api.dugapay.com";
...
async function raw(path, { method = "GET", body, retried = false } = {}) {
  // If there's currently no token in memory at all and this isn't one of
  // the auth endpoints themselves (which don't need one), this request
  // is guaranteed to fail with "missing access token" — try restoring
  // the session first rather than sending it anyway and only recovering
  // after that failure. retried guards against recursing indefinitely,
  // same flag the reactive 401-retry below already uses.
  if (!accessToken && !retried && !NO_RETRY_PATHS.includes(path)) {
    await tryRefresh();
  }
...
export const adminApi = {
  ...
  users: {
    ...
    downloadStatementCsv: (id, from, to) => downloadFile(`/admin/users/${id}/statement?${qs({ from, to, format: "csv" })}`, `mitapesa-statement-${id}.csv`),
  ...
  staff: {
    list: () => raw("/admin/staff").then((d) => d.staff),
    create: (data) => raw("/admin/staff", { method: "POST", body: data }).then((d) => d.user),
    remove: (id) => raw(`/admin/staff/${id}`, { method: "DELETE" }),
  },
  ...
  // This calendar month's actual spend and usage for the given AI
  // feature ("voice" or "analytics", defaults to "voice") — same backend
  // endpoint the mobile app's own Admin portal already reads.
  // The combined picture across every AI feature sharing the OpenAI
  // account — total real spend (converted to TZS), total revenue, and
  // the actual net position (revenue minus cost), both per feature and
  // combined.
  aiUsageSummary: () => raw("/admin/ai-usage/summary"),
  aiUsage: (feature) => raw(`/admin/ai-usage${feature ? `?feature=${feature}` : ""}`),
  // A specific customer's full voice-credit picture for the given
  // feature — total purchased, used, remaining, and every individual
  // purchase.
  voiceCredits: (userId, feature) => raw(`/admin/voice-credits/${userId}${feature ? `?feature=${feature}` : ""}`),
  // Marks a customer's unused purchased credits as fully consumed, for
  // clearing out leftover test-purchase credits — super admin only.
  clearVoiceCredits: (userId, feature) => raw(`/admin/voice-credits/${userId}/clear${feature ? `?feature=${feature}` : ""}`, { method: "POST" }),

  // First-use education content shown to customers, in-app — the
  // post-signup welcome tour (tourStep set) and per-screen banner tips
  // (tourStep null). Text lives here in the database specifically so it
  // can be corrected — especially the Swahili, for how it's actually
  // spoken regionally — without an app update or code deploy.
  onboardingTips: {
    list: () => raw("/admin/onboarding-tips").then((d) => d.tips),
    create: (data) => raw("/admin/onboarding-tips", { method: "POST", body: data }),
    update: (key, data) => raw(`/admin/onboarding-tips/${key}`, { method: "PATCH", body: data }),
  ...
  fees: {
    ...
    bundles: {
      ...
      processRenewals: () => raw("/admin/fees/bundles/process-renewals", { method: "POST" }),
    },
    exemptions: {
    ...
    downloadReportCsv: (from, to) => downloadFile(`/admin/fees/report?${qs({ from, to, format: "csv" })}`, "mitapesa-fee-revenue.csv"),
    downloadReportDetailCsv: (from, to) => downloadFile(`/admin/fees/report/detail?${qs({ from, to, format: "csv" })}`, "mitapesa-fee-transactions.csv"),
  },

  reconciliation: {
    run: (date) => raw("/admin/reconciliation/run", { method: "POST", body: { date } }),
    exceptions: (status, page = 1, pageSize = 25) => raw(`/admin/reconciliation/exceptions?${qs({ status, page, pageSize })}`),
    updateException: (id, data) => raw(`/admin/reconciliation/exceptions/${id}`, { method: "PATCH", body: data }),
  },

  exchangeRates: {
    list: () => raw("/admin/exchange-rates").then((d) => d.rates),
    upsert: (currency, rateToTZS) => raw(`/admin/exchange-rates/${currency}`, { method: "PUT", body: { rateToTZS } }).then((d) => d.rate),
    remove: (currency) => raw(`/admin/exchange-rates/${currency}`, { method: "DELETE" }),
  },
};
