      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
            <StatCard label="MitaPesa fee revenue" value={fmtTZS(report.totalMitaPesaFee)} tone="accent" />
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-1.5">
            ...
            <StatCard label="AI features (voice logging)" value={fmtTZS(report.voiceCreditRevenue)} tone="gold" hint={`${report.voiceCreditPurchaseCount} purchased`} />
            <StatCard label="AI features (Ask AI analytics)" value={fmtTZS(report.analyticsCreditRevenue)} tone="gold" hint={`${report.analyticsCreditPurchaseCount} purchased`} />
          </div>
          <p className="text-[11.5px] text-inkFaint mb-8">"AI features" is revenue from customers buying more usage beyond their free monthly allowance, for each AI feature separately — see the AI Usage page for spend, and per-customer purchase history.</p>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-border bg-bgSoft/60">
                  ...
                  <th className="px-5 py-3 text-[11px] font-semibold text-inkFaint uppercase tracking-wide">MitaPesa fee</th>
              ...
              <tbody>
                {report.byTransactionType.map((r) => (
                  <tr key={r.transactionTypeCode} className="border-b border-border last:border-0">
                    ...
                    <td className="px-5 py-3.5 text-[12.5px] text-inkSoft font-mono">{fmtTZS(r.mitaPesaFee)}</td>
