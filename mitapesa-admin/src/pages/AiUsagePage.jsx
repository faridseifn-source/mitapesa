...
function CreditLookup({ user, initialUserId }) {
  ...
  const loadSummary = (userId) => {
    ...
    adminApi.voiceCredits(userId).then(setSummary).catch((err) => setError(err.message)).finally(() => setLoadingSummary(false));
  };
  ...
  const handleClear = async () => {
    ...
    try { await adminApi.clearVoiceCredits(selectedId); loadSummary(selectedId); }
  ...
  return (
    <Card className="p-5 mb-8">
      <p className="text-[13px] font-semibold text-ink mb-1">Look up a customer's voice credits</p>
          ...
          {loadingSummary ? (
            ...
            <>
              ...
              {summary.purchases.length === 0 ? (
                <EmptyState title="No purchases yet" sub="This customer hasn't bought any voice credits." />
              ) : (
...
export default function AiUsagePage({ user }) {
  ...
  return (
    <div>
      ...
      <CreditLookup user={user} initialUserId={lookupUserId} />
